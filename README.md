...
- 移除系统分享、`a[download]`、Blob、文件选择器等多级降级方案，逻辑大幅简化
- 全应用仅保留「头像上传」一处文件操作，已在手机网页、手机 PWA、电脑浏览器、电脑 PWA 四种模式下验证可用
- 电脑端支持拖拽图片设置头像
- 头像永久排除出备份，导出体积显著缩小（实测 30 KB → 1 KB）
- 导入时自动沿用本机已有头像，不会因导入而丢失

### v1.5（2026-09-17）

- 头像压缩优化：输出 144 px 并自适应降质，4000×3000 原图可压至约 10 KB
- 修复 PNG 透明图转 JPEG 后出现黑底的问题
- 补全上传容错：超大图提前拦截、读取失败与格式不支持均有明确提示
- 新增「含头像 / 不含头像」导出选项（v1.6 已简化为默认不含头像）

### v1.4（2026-09-17）

- 修复 PWA 独立模式（从桌面图标打开）下无法导入导出的问题
- 修复文件选择器因未插入 DOM 而不响应的问题
- 桌面单独适配：桌面 PWA 走原生下载，不走系统分享
- 新增「粘贴文本导入」兜底通道
- 延长下载链接清理延时，修复大文件偶发下载失败

### v1.3

- 持久化存储申请，降低数据被系统回收的概率
- 备份提醒：从未备份或超 14 天未备份时首页提示
- 新增数据保护面板，显示持久化状态、上次备份时间与占用空间
